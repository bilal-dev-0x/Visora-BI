...
class DataAnalyzer:
    ...
    def get_other_details(self):
        self.product_names = self.df["Product Name"].dropna().unique().tolist()
        self.categories = self.df["Category"].dropna().unique().tolist()
        self.regions = self.df["Region"].dropna().unique().tolist()
        self.earliest_date = str(self.df["Order Date"].min().date())
        self.latest_date = str(self.df["Order Date"].max().date())
    ...
    def build_report(self):
        self.summary = {
            date.today().isoformat(): {
                ...
                "Other Details": {
                    "Unique Product Names": self.product_names,
                    "Unique Categories": self.categories,
                    "Unique Regions": self.regions,
