"""Standalone validation for Day 4: unified orchestration
...
SANDBOX = Path("/tmp/visora_day4_sandbox")
PASS, FAIL = [], []
...
def main():
    ...
    check("Day4: normal dataset ingests successfully", ingest_result["ingested"])
    ...
    check("Day4: unified report has status success", report.get("status") == "success")
    check("Day4: unified report carries dataset/metrics/trends/contribution/anomalies",
          all(key in report for key in ("dataset", "metrics", "trends", "contribution", "anomalies")))
    check("Day4: unified report carries evidence layer", "evidence" in report and isinstance(report["evidence"], list))
    check("Day4: unified report carries prioritized_findings", "prioritized_findings" in report)
    check("Day4: unified report carries structured ai_analysis",
          "ai_analysis" in report and "summary" in report["ai_analysis"] and "insights" in report["ai_analysis"])
    check("Day4: JSON-serializable end to end", bool(json.dumps(report)))
    ...
    check("Day4: evidence includes at least one anomaly or trend or concentration item",
    ...
    check("Day4: prioritized findings are sorted critical->low", priorities == sorted(priorities))
    ...
    check("Day4: current unified JSON report file was written", current_json_path.exists())

    dataset_row = registry.get_dataset(dataset_id)
    txt_path = report_store.txt_report_path(dataset_row, reports_dir)
    check("Day4: dataset-specific TXT report file was written", txt_path.exists())
    check("Day4: TXT report filename is NOT the generic 'analysis.txt'", txt_path.name != "analysis.txt")
    check("Day4: TXT report filename is dataset-specific (not a shared name)",
    ...
    check("Day4: re-analyzing the same dataset does not accumulate a second TXT file",
    ...
    check("Day4: second dataset gets its OWN, differently-named TXT report",
          txt_path_2.exists() and txt_path_2 != txt_path)
    check("Day4: dataset B report status is success", report_2.get("status") == "success")
    ...
    check("Day4: delete_dataset_and_report returns the deleted dataset row", deleted is not None)
    check("Day4: deleting dataset A removes its TXT report", not txt_path.exists())
    check("Day4: deleting dataset A does NOT remove dataset B's TXT report", txt_path_2.exists())
    check("Day4: deleting dataset A removes it from the registry", registry.get_dataset(dataset_id) is None)
    check("Day4: deleting dataset A leaves dataset B in the registry", registry.get_dataset(dataset_id_2) is not None)

    missing_result = pipeline.delete_dataset_and_report("not-a-real-id", registry=registry, db_file=db_file)
    check("Day4: deleting a nonexistent dataset_id returns None instead of raising", missing_result is None)
    ...
    check("Day4: analyzing a missing dataset_id returns an error report, not an exception",
    ...
    for filename, df in edge_cases.items():
        ...
        check(f"Day4 edge case ({filename}): pipeline never raises", not crashed)
        if not crashed:
            check(f"Day4 edge case ({filename}): result has a status field",
                  edge_report is not None and "status" in edge_report)
            check(f"Day4 edge case ({filename}): result is JSON-serializable", bool(json.dumps(edge_report)))
    ...
    check("Day4 evidence: extreme missingness produces quality evidence",
          any(item["type"] == "quality" and item.get("issue") == "missing_values" for item in evidence))
    check("Day4 evidence: extreme duplicates produces quality evidence",
          any(item["type"] == "quality" and item.get("issue") == "duplicate_rows" for item in evidence))
    check("Day4 evidence: strong growth produces trend evidence",
          any(item["type"] == "trend" for item in evidence))
    check("Day4 evidence: high concentration produces concentration evidence",
          any(item["type"] == "concentration" for item in evidence))
    check("Day4 evidence: strong anomaly produces anomaly evidence",
          any(item["type"] == "anomaly" for item in evidence))
    check("Day4 evidence: every item has a stable id", all("id" in item for item in evidence))

    prioritized = prioritization_mod.prioritize_evidence(evidence)
    check("Day4 prioritization: z=4.5 anomaly is Critical",
          next(item for item in prioritized if item["type"] == "anomaly")["priority"] == "critical")
    check("Day4 prioritization: 60% missing is Critical",
          next(item for item in prioritized if item.get("issue") == "missing_values")["priority"] == "critical")
    check("Day4 prioritization: 85% concentration is Critical",
    ...
    check("Day4 prioritization: output is sorted critical->low", priorities_sorted == sorted(priorities_sorted))

    empty_evidence = evidence_mod.build_evidence({})
    check("Day4 evidence: malformed/empty context returns [] instead of raising", empty_evidence == [])
    check("Day4 prioritization: empty evidence returns []", prioritization_mod.prioritize_evidence([]) == [])
    ...
    check("Day4 structured AI: no provider -> local_fallback source", structured.source == "local_fallback")
    check("Day4 structured AI: local fallback still produces insights from evidence",
          len(structured.result["insights"]) == len(prioritized))
    check("Day4 structured AI: local fallback result is JSON-serializable", bool(json.dumps(structured.result)))
    ...
    check("Day4 structured AI: oversized context skips AI (size_threshold_exceeded)",
          oversized.ai_skipped_reason == "size_threshold_exceeded" and not oversized.ai_attempted)
    check("Day4 structured AI: oversized context never touches the provider chain", not mocked_chain.called)
    ...
    check("Day4 structured AI: malformed provider JSON falls back safely (no crash)",
    ...
    check("Day4 structured AI: well-formed JSON is parsed as source=ai", good_result.source == "ai")
    check("Day4 structured AI: insight priority is pinned to the evidence's own deterministic priority, not the model's",
          good_result.result["insights"][0]["priority"] ==
          next(item for item in prioritized if item["id"] == good_evidence_id)["priority"])
    valid_ids = {item["id"] for item in evidence}
    check("Day4 structured AI: evidence attached to insights always resolves to a real evidence item",
    ...
    check("Day4 structured AI: markdown-fenced JSON is still parsed correctly", fenced_result.source == "ai")
    ...
    check("Day4 structured AI: provider timeout falls back to local, deterministic BI still works",
          timeout_result.source == "local_fallback" and timeout_result.ai_attempted)

    # ------------------------------------------------------------------
    # 11. Chart type selection -- data-aware validation
    # ------------------------------------------------------------------
    check("Day4 chart: Line supported with date+numeric",
          chart_selection.is_chart_type_supported("Line", ["Order Date"], [], ["Sales"]))
    check("Day4 chart: Line NOT supported without a date column",
          not chart_selection.is_chart_type_supported("Line", [], ["Category"], ["Sales"]))
    check("Day4 chart: Bar supported with category+numeric",
          chart_selection.is_chart_type_supported("Bar", [], ["Category"], ["Sales"]))
    check("Day4 chart: Scatter requires two numeric columns",
    ...
    check("Day4 chart: invalid selection returns a clear reason instead of crashing",
          not supported and isinstance(reason, str) and reason)
    check("Day4 chart: default suggestion is Line for date+numeric schema",
          chart_selection.suggest_default_chart_type(["Order Date"], [], ["Sales"]) == "Line")
    check("Day4 chart: default suggestion is Bar for category+numeric schema",
    ...
    # ------------------------------------------------------------------
    dashboard_sandbox = Path("/tmp/visora_day4_dashboard_sandbox")
    ...
    try:
        ...
        app.run()
        check("Day4 dashboard: runs cleanly with no datasets", len(app.exception) == 0)
        ...
        app2.run()
        check("Day4 dashboard: runs cleanly with a real dataset selected", len(app2.exception) == 0)

        subheaders = [element.value for element in app2.subheader] if hasattr(app2, "subheader") else []
        check("Day4 dashboard: shows a Prioritized Findings section", any("Prioritized Findings" in text for text in subheaders))
        check("Day4 dashboard: shows a Reports section (TXT download)", any("Reports" in text for text in subheaders))
        check("Day4 dashboard: shows a Chart Explorer section", any("Chart Explorer" in text for text in subheaders))

        txt_written = list((dashboard_sandbox / "reports").glob("sales_*.txt"))
        check("Day4 dashboard: selecting a dataset writes its dataset-specific TXT report", len(txt_written) == 1)
    finally:
        os.chdir(old_cwd)
        sys.path[:] = old_path
        for mod_name in list(sys.modules):
